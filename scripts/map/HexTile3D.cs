...
namespace Natiolation.Map
{
    ...
    public partial class HexTile3D : Node3D
    {
        ...
        {
            // Domo principal de la colina
            var grassMat  = SolidMat(_baseColor.Lightened(0.10f));
            var rockMat   = SolidMat(new Color(0.52f, 0.48f, 0.40f), roughness: 0.88f);
            var darkGrass = SolidMat(_baseColor.Darkened(0.18f));

            // Cuerpo central de la colina (esfera aplastada grande)
            var dome = new MeshInstance3D
            {
                Mesh             = new SphereMesh { Radius=2.1f, Height=2.8f, RadialSegments=14, Rings=8 },
                MaterialOverride = grassMat,
                Position         = new Vector3(0f, h - 0.6f, 0f),
                Scale            = new Vector3(1.0f, 0.62f, 1.0f),
            };
            AddChild(dome);

            // Montículo secundario para romper simetría
            AddChild(new MeshInstance3D
            {
                Mesh             = new SphereMesh { Radius=1.2f, Height=1.6f, RadialSegments=10, Rings=5 },
                MaterialOverride = darkGrass,
                Position         = new Vector3(0.8f, h - 0.4f, 0.6f),
                Scale            = new Vector3(1.0f, 0.55f, 1.0f),
            });

            // Algunas rocas en la cima
            int seed = Q * 13 ^ R * 7;
            (Vector3 p, float s)[] rocks = {
                (new(-0.5f, h + 0.6f,  0.2f), 0.22f + (seed & 3) * 0.04f),
                (new( 0.4f, h + 0.5f, -0.3f), 0.18f + ((seed>>2) & 3) * 0.03f),
                (new( 0.1f, h + 0.7f,  0.5f), 0.15f),
            };
            foreach (var (p, s) in rocks)
                AddChild(MeshAt(new SphereMesh { Radius=s, RadialSegments=6, Rings=3 }, rockMat, p));
        }
